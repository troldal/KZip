...
namespace Catch {
    ...
    template<typename T, typename... Rest> inline constexpr auto is_unique<T, Rest...> = std::bool_constant<(!std::is_same_v<T,
                                                                                                                             Rest> && ...) && is_unique<
            Rest...>>{};
...
namespace Catch {
    ...
    struct is_range {
        static const bool value = !std::is_same<decltype(begin(std::declval<T>())),
                                                not_this_one>::value && !std::is_same<decltype(end(std::declval<T>())),
                                                                                      not_this_one>::value;
    };
...
namespace Catch {
    namespace Generators {
        ...
        GeneratorWrapper<T> filter(Predicate&& pred, GeneratorWrapper<T>&& generator) {
            ...
            return GeneratorWrapper<T>(std::unique_ptr<IGenerator<T>>(pf::make_unique<FilterGenerator<T, Predicate>>(std::forward<
                    Predicate>(pred), std::move(generator))));
        }
        ...
        GeneratorWrapper<T> map(Func&& function, GeneratorWrapper<T>&& generator) {
            ...
            return GeneratorWrapper<T>(pf::make_unique<MapGenerator<T, T, Func>>(std::forward<Func>(function),
                                                                                 std::move(generator)));
        }
...
namespace Catch {
    class WildcardPattern {
        enum WildcardPosition {
            NoWildcard = 0, WildcardAtStart = 1, WildcardAtEnd = 2, WildcardAtBothEnds = WildcardAtStart | WildcardAtEnd
        };
...
namespace Catch {
    namespace TestCaseTracking {
        ...
        class TrackerContext {
            ...
            enum RunState {
                NotStarted, Executing, CompletedCycle
            };
        ...
        protected:
            enum CycleState {
                NotStarted, Executing, ExecutingChildren, NeedsAnotherRun, CompletedSuccessfully, Failed
            };
...
namespace Catch {
    namespace Detail {
        ...
        void Approx::setMargin(double margin) {
            ...
            CATCH_ENFORCE(margin >= 0, "Invalid Approx::margin: " << margin << '.' << " Approx::Margin has to be non-negative.");
            m_margin = margin;
        }
        ...
        void Approx::setEpsilon(double epsilon) {
            ...
            CATCH_ENFORCE(epsilon >= 0 && epsilon <= 1.0,
                          "Invalid Approx::epsilon: " << epsilon << '.' << " Approx::epsilon has to be in [0, 1]");
            m_epsilon = epsilon;
        }
            ...
            public:
                class iterator {
                    ...
                    auto isBoundary(size_t at) const -> bool {
                        ...
                        return at == line().size() || (isWhitespace(line()[at]) && !isWhitespace(line()[at - 1])) || isBreakableBefore(
                                line()[at]) || isBreakableAfter(line()[at - 1]);
                    }
        ...
        namespace detail {
            ...
            struct Parser : ParserBase {
                ...
                void writeToStream(std::ostream& os) const {
                    ...
                    for (auto const& cols : rows) {
                        auto row = TextFlow::Column(cols.left).width(optWidth).indent(2) + TextFlow::Spacer(4) + TextFlow::Column(
...
namespace Catch {
    ...
    clara::Parser makeCommandLineParser(ConfigData& config) {
        ...
        auto cli = ExeName(config.processName) | Help(config.showHelp) | Opt(config.listTests)["-l"]["--list-tests"](
                "list all/matching test cases") | Opt(config.listTags)["-t"]["--list-tags"]("list all/matching tags") | Opt(config.showSuccessfulTests)["-s"]["--success"](
                "include successful tests in output") | Opt(config.shouldDebugBreak)["-b"]["--break"](
                "break into debugger on failure") | Opt(config.noThrow)["-e"]["--nothrow"]("skip exception tests") | Opt(config.showInvisibles)["-i"]["--invisibles"](
                "show invisibles (tabs, newlines)") | Opt(config.outputFilename,
                                                          "filename")["-o"]["--out"]("output filename") | Opt(setReporter,
                                                                                                              "name")["-r"]["--reporter"](
                "reporter to use (defaults to console)") | Opt(config.name,
                                                               "name")["-n"]["--name"]("suite name") | Opt([&](bool) { config.abortAfter = 1; })["-a"]["--abort"](
                "abort at first failure") | Opt([&](int x) { config.abortAfter = x; }, "no. failures")["-x"]["--abortx"](
                "abort after x failures") | Opt(setWarning,
                                                "warning name")["-w"]["--warn"]("enable warnings") | Opt([&](bool flag) { config.showDurations = flag ? ShowDurations::Always : ShowDurations::Never; },
                                                                                                         "yes|no")["-d"]["--durations"](
                "show test durations") | Opt(loadTestNamesFromFile, "filename")["-f"]["--input-file"](
                "load test names to run from a file") | Opt(config.filenamesAsTags)["-#"]["--filenames-as-tags"](
                "adds a tag for the filename") | Opt(config.sectionsToRun, "section name")["-c"]["--section"](
                "specify section to run") | Opt(setVerbosity,
                                                "quiet|normal|high")["-v"]["--verbosity"]("set output verbosity") | Opt(config.listTestNamesOnly)["--list-test-names-only"](
                "list all/matching test cases names only") | Opt(config.listReporters)["--list-reporters"]("list all reporters") | Opt(
                setTestOrder,
                "decl|lex|rand")["--order"]("test case order (defaults to decl)") | Opt(setRngSeed,
                                                                                        "'time'|number")["--rng-seed"](
                "set a specific seed for random numbers") | Opt(setColourUsage, "yes|no")["--use-colour"](
                "should output be colourised") | Opt(config.libIdentify)["--libidentify"](
                "report name and version according to libidentify standard") | Opt(setWaitForKeypress,
                                                                                   "start|exit|both")["--wait-for-keypress"](
                "waits for a keypress before exiting") | Opt(config.benchmarkResolutionMultiple,
...
namespace Catch {
    ...
    std::size_t listReporters() {
        ...
        for (auto const& factoryKvp : factories) {
            Catch::cout()
                    << Column(factoryKvp.first + ":").indent(2).width(5 + maxNameLen) + Column(factoryKvp.second->getDescription()).initialIndent(
                            0).indent(2).width(CATCH_CONFIG_CONSOLE_WIDTH - maxNameLen - 8) << "\n";
        }
        Catch::cout() << std::endl;
...
namespace Catch {
    ...
    Session::Session() {
        ...
        if (alreadyInstantiated) {
            CATCH_TRY { CATCH_INTERNAL_ERROR("Only one instance of Catch::Session can ever be used"); } CATCH_CATCH_ALL { getMutableRegistryHub().registerStartupException(); }
        }
    ...
    void Session::libIdentify() {
        ...
        Catch::cout() << std::left << std::setw(16) << "description: " << "A Catch test executable\n" << std::left
                      << std::setw(16) << "category: " << "testframework\n" << std::left << std::setw(16) << "framework: "
    ...
    std::string TagAliasRegistry::expandAliases(std::string const& unexpandedTestSpec) const {
        ...
        for (auto const& registryKvp : m_registry) {
            ...
            if (pos != std::string::npos) {
                expandedTestSpec = expandedTestSpec.substr(0,
                                                           pos) + registryKvp.second.tag + expandedTestSpec.substr(pos + registryKvp.first.size());
            }
        }
...
namespace Catch {
    ...
    void setTags(TestCaseInfo& testCaseInfo, std::vector<std::string> tags) {
        ...
        for (auto const& tag : tags) {
            ...
            testCaseInfo.properties = static_cast<TestCaseInfo::SpecialProperties>( testCaseInfo.properties | parseSpecialTag(
                    lcaseTag));
...
namespace Catch {
    namespace TestCaseTracking {
        ...
        ITracker& TrackerContext::startRun() {
            ...
            m_rootTracker    = std::make_shared<SectionTracker>(NameAndLocation("{root}", CATCH_INTERNAL_LINEINFO),
                                                                *this,
                                                                nullptr);
        ...
        bool SectionTracker::isComplete() const {
            ...
            if ((m_filters.empty() || m_filters[0] == "") || std::find(m_filters.begin(),
                                                                       m_filters.end(),
    ...
    namespace {
        ...
        void printTotals(std::ostream& out, const Totals& totals) {
            ...
            if (totals.testCases.total() == 0) {
                ...
                const std::string qualify_assertions_failed = totals.assertions.failed == totals.assertions.total() ? bothOrAll(
                        totals.assertions.failed) : std::string();
...
namespace Catch {
    ...
    namespace {
        ...
        class Duration {
            enum class Unit {
                Auto, Nanoseconds, Microseconds, Milliseconds, Seconds, Minutes
            };
    ...
    public:
        ...
        friend TablePrinter& operator<<(TablePrinter& tp, ColumnBreak) {
            ...
            auto padding = (strSize + 2 < static_cast<std::size_t>(colInfo.width)) ? std::string(colInfo.width - (strSize + 2),
                                                                                                 ' ') : std::string();
    ...
    void ConsoleReporter::printTotals(Totals const& totals) {
        ...
        if (totals.testCases.total() == 0) {
            ...
            columns.push_back(SummaryColumn("passed",
                                            Colour::Success).addRow(totals.testCases.passed).addRow(totals.assertions.passed));
            columns.push_back(SummaryColumn("failed",
                                            Colour::ResultError).addRow(totals.testCases.failed).addRow(totals.assertions.failed));
            columns.push_back(SummaryColumn("failed as expected",
                                            Colour::ResultExpectedFailure).addRow(totals.testCases.failedButOk).addRow(totals.assertions.failedButOk));
...
namespace Catch {
    ...
    void XmlReporter::testCaseStarting(TestCaseInfo const& testInfo) {
        ...
        m_xml.startElement("TestCase").writeAttribute("name", trim(testInfo.name)).writeAttribute("description",
                                                                                                  testInfo.description).writeAttribute(
                "tags",
                testInfo.tagsAsString());
    ...
    bool XmlReporter::assertionEnded(AssertionStats const& assertionStats) {
        ...
        if (result.hasExpression()) {
            m_xml.startElement("Expression").writeAttribute("success", result.succeeded()).writeAttribute("type",
    ...
    void XmlReporter::testGroupEnded(TestGroupStats const& testGroupStats) {
        ...
        m_xml.scopedElement("OverallResults").writeAttribute("successes", testGroupStats.totals.assertions.passed).writeAttribute(
                "failures",
                testGroupStats.totals.assertions.failed).writeAttribute("expectedFailures",
    ...
    void XmlReporter::testRunEnded(TestRunStats const& testRunStats) {
        ...
        m_xml.scopedElement("OverallResults").writeAttribute("successes", testRunStats.totals.assertions.passed).writeAttribute(
                "failures",
                testRunStats.totals.assertions.failed).writeAttribute("expectedFailures",
